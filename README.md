...
- **9月始まり・8月締め**の会計期（59期＝2026/9〜2027/8）に対応。案件は部門ごとに**計上月**で管理します。
- 3段階の権限（システム管理者／管理者＝部門長／一般）と、ユーザーと部門の紐づけ（複数部門可）。
- 案件の閲覧・登録・編集・エクスポート（CSV・Excel・PDF）と、**数字まとめ**（予実・損益・カテゴリー別・確度別を月／四半期／半期／通期で集計）。
- データは1つのファイル（SQLite）に保存され、そのファイルをコピーするだけでバックアップできます。

> **権限の考え方**
> - **システム管理者**: 全権。ユーザー管理・マスタ管理を含むすべての操作が可能。
> - **管理者（部門長）**: 全部門の案件を閲覧でき、編集できるのは自分が紐づく部門のみ。
> - **一般ユーザー**: 自部門の案件を閲覧でき、編集できるのは自分が担当の案件のみ。
>
...
6. 「カテゴリー管理」から部門ごとのカテゴリーを追加・編集できます。

### 案件管理表
上部メニュー「案件管理」で、部門を選ぶとその部門の案件が**計上月ごと**にまとまって表示されます
（各月に売上・仕入・売上総利益の小計）。「＋ 案件を登録」で
担当者／区分／カテゴリー／案件名／確度／売上／仕入／見込み・対応工数／備考を入力します
（売上総利益は「売上−仕入」で自動計算）。右上の **CSV / Excel / PDF** で出力できます。
...
表示切替ができます。確度・区分・部門の初期値は `app/cli.py` の `seed-masters` に定義しています。
