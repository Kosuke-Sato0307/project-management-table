...
> **現在の実装状況**: 主要機能は一通り完成（基盤＋認証／案件の一覧・検索・登録・編集・削除／
> マスタ管理／エクスポート CSV・Excel・PDF／インポート Excel・CSV）。仕上げ・UI調整を継続中です。
>
...
- データベースのテーブル作成
- 案件ステータス（進行中／受注／失注／完成）・確度ランク（○／A／B／C／D／E／×）・部署（第1営業部 ほか）の初期値投入
...
### 案件データの一括取込（インポート）
上部メニュー「インポート」から、Excel/CSVでまとめて登録できます。

1. 「Excelテンプレート」または「CSVテンプレート」をダウンロード
2. テンプレートの見出しはそのままに、2行目以降にデータを入力
3. ファイルを選び「内容を確認する（プレビュー）」→ 件数と内容を確認 → 「取り込む」
4. **不正な行が1つでもあると、エラー内容を表示してすべて取り込みません**（安全のため全件まとめて成否判定）。修正して再度お試しください。
5. 既に同じ案件番号がある場合は、確定時に「スキップ／上書き更新」を選べます。

> 日付は `2026-07-01`（YYYY-MM-DD）、完成月は `2026-09`（YYYY-MM）、金額は数字、
> ステータス・確度は**マスタに登録済みの名称**で入力してください。Shift-JISのCSVも取り込めます。

### エクスポート
「案件一覧」右上の **CSV / Excel / PDF** から、その時の検索条件で出力できます。
...
### 選択肢（ステータス・確度ランク・部署）を増やしたいとき

管理者でログインし、上部メニュー「マスタ管理」から追加・編集・表示切替ができます
（案件フォームやインポートのテンプレートに即反映されます）。初期値は `app/cli.py` の `seed-masters` に定義しています。
